...
# Source early in grok_worker.sh / dispatcher.sh / runners / services: (jules_worker removed)
...
if [ $FORCE_RESTART -eq 1 ]; then
    ...
    log "Worker restart skipped (use --force-restart only on low-load). Manual: systemctl --user restart agentforge-worker"
fi
