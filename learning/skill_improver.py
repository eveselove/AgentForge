...
if __name__ == "__main__":
    print(_MESSAGE)
    # task-5af0e350: direct remaining to runner if possible (candidate promote etc)
    import os
    import sys
    ...
    if os.path.isfile(runner) and os.access(runner, os.X_OK):
        args = [runner, "candidate", "promote"] + sys.argv[1:]
        print(f"  Directing skill_improver to pure runner: {' '.join(args)}")
        try:
