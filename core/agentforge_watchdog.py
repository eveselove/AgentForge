...
import os
for p in [
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),  # agentforge root при запуске из core/
...
def analyze_log_for_loops(log_path):
    ...
    try:
        ...
        recent_lines = [l.strip() for l in lines[-60:] if l.strip()]  # Увеличено до 60 для лучшего покрытия
        if not recent_lines:
            return None
        ...
        tool_calls = [l for l in recent_lines if any(kw in l for kw in ["Running tool", "Command output", "tool call", "invoke tool", "execute", "subprocess"])]
...
def guardian_loop():
    ...
    if not get_last_checkpoint:
        log("⚠️ Guardian: get_last_checkpoint недоступен (нет task_checkpoints) — RAG/self-heal ограничен")
        return
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            if reviewed > 0:
                ...
                log(f"🛡️ Auto-review sweep: 0 задач в review (цикл {_review_cycle_counter})")
    except Exception as e:
        log(f"⚠️ Auto-review sweep failed: {e}")

def main():
    log("🚀 Запуск AgentForge Watchdog + Guardian (core/agentforge_watchdog.py)")
    log("   Мониторинг: log loop heuristics + token saving kills + Guardian self-healing (RAG retries) + periodic sweeps")
    while True:
        try:
            ...
            for task in tasks:
                ...
                if reason:
                    log(f"🚨 Обнаружена аномалия в {task_id}: {reason}. Принимаю меры (token-saving kill)...")
