...
2. **Используйте git worktree** для изоляции (каждая задача в `/tmp/agentforge/TASK_ID`):
   ...
   git -C /home/eveselove/planlytasksko worktree add "$WORK_DIR" -b "agentforge/$TASK_ID" 2>/dev/null
   ```
...
**Обходной путь:** Мониторьте логи в `/home/eveselove/agentforge/logs/`.
...
|------------|----------|----------|
| `AGENTFORGE_RUST_RUNNER` | Путь к бинарнику | `/home/eveselove/agentforge/rust/target/release/agentforge-runner` |
