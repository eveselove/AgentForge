...
На сервере есть готовые скрипты для массового создания задач:
    ...
    ssh agx@146.120.89.199 "cd ~/agentforge && python3 create_tasks.py"

    # Создание задач на архитектуру
    ssh agx@146.120.89.199 "cd ~/agentforge && python3 create_arch_tasks.py"

---

## 4. Целевое состояние: agentforge-runner task CLI (планируется)

> ⚠️ CLI для управления задачами через agentforge-runner ещё НЕ реализован.
...
| Jetson AGX | `agx@146.120.89.199` | Основной сервер |
| MCP-сервер | Через Antigravity IDE | agentforge_* инструменты |

### SSH-туннель для дашборда

    ssh -L 9090:localhost:8080 agx@146.120.89.199
