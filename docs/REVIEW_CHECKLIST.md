...
- [ ] **Diff audit against last-wave defect classes** (the most common findings):
  ...
  - [ ] Correct data sources in hooks and capture logic. Traceability is now in commit-msg hook (receives $1=msg file from git at correct post-prepare phase). Pre-commit does not (and must not) validate msg using git log -1 or EDITMSG (too early for -m "task <id>" and worktrees). See bin/commit-msg + bin/validate-commit-msg (post MICRO-05/post-bypass cleanup).
