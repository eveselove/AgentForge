...
### Вторичные (обнаруженные на Jetson)
| Локальный путь | Возможный GitHub-репо | Описание |
|----------------|----------------------|----------|
| `/home/agx/planlytasksko/chromimic` | `eveselove/chromimic` | Подпроект chromimic |
| `/home/agx/planlytasksko/turboquant-vllm` | `eveselove/turboquant-vllm` | vLLM-форк для квантования |
| `/home/agx/planlytasksko/blutter` | `eveselove/blutter` | Flutter RE-инструмент |

> ⚠️ **Важно:** Jules работает с GitHub-репозиториями через облако Google.
> Локальные пути на Jetson не используются напрямую — нужен валидный `owner/repo` на GitHub.
...
- ❌ `/home/agx/planlytasksko` (локальный путь)
...
```bash
git -C "/home/agx/planlytasksko" worktree add "$WORKTREE_DIR" -b "agentforge/$TASK_ID"
```
