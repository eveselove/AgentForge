...
export PATH=$HOME/.nvm/versions/node/v20.20.1/bin:$HOME/.cargo/bin:$HOME/bin:$PATH
...
POLL_INTERVAL=15
RUNNER="$HOME/agentforge/agents/jules_runner.sh"
...
while true; do
    # Получаем все задачи
    TASKS=$(curl -H "Authorization: Bearer $AGENTFORGE_API_KEY" -s "$API/tasks" 2>/dev/null)
    if [ -z "$TASKS" ]; then
        sleep $POLL_INTERVAL
        continue
    fi
