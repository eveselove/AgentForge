...
- `/home/agx/agentforge/rust/crates/agentforge-core/` — Outcome, Task, Agent primitives (unified canonical source of truth)
- `/home/agx/agentforge/rust/crates/agentforge-learning/` — TrajectoryDataset, SkillImprover, trainers (DPO/KTO/SFT skeletons), types + training_ready/ (preference_pairs.jsonl, kto_examples.jsonl, sft_success_trajectories.jsonl)
- `/home/agx/agentforge/rust/crates/agentforge-planning/` — HierarchicalPlanner + LongTaskManager
- `/home/agx/agentforge/rust/crates/agentforge-safety/` — PolicyEngine + 4+ policies
- `/home/agx/agentforge/rust/crates/agentforge-observability/` — Spans, replay, PRM integration
- `/home/agx/agentforge/rust/crates/agentforge-long-horizon/` — Long horizon task mgmt + examples
- `/home/agx/agentforge/rust/crates/agentforge-runner/` — Full CLI: demo, stats, flywheel-export (--rich --json), improve-skill, planning/safety/obs paths
- **Production binary:** `/home/agx/agentforge/rust/target/release/agentforge-runner` (860888 bytes, stripped aarch64 ELF, all subcommands + rich flywheel-export confirmed operational)

### Python Bridges, Hooks & Integration Layer (Full Farm Wiring)
- `/home/agx/agentforge/rust_flywheel_step.py` — canonical step (rich Rust export preferred, auto-ingest to pending)
- `/home/agx/agentforge/enable_rust_flywheel.py` — one-command activate() with idempotent post_process monkey-patch
- `/home/agx/agentforge/phase2_3_integration.py` — guard point for flywheel
- `/home/agx/agentforge/eval/post_process.py` — PRM sidecars + Rust pairs + rate-limited flywheel calls
- `/home/agx/agentforge/learning/trajectory_dataset.py` — rich ingest, export_preference_pairs_via_rust, outcome normalization to Rust canonical
- `/home/agx/agentforge/learning/pending_candidates.py` — ingest, list_high_value_candidates (prioritizer), promote_candidate + promote-and-ab, meta, AUTONOMY notes
- `/home/agx/agentforge/learning/evaluator.py` — LearningEvaluator + ABTestConfig + ab_test_skill_versions + recorder (sim + real paths)
- `/home/agx/agentforge/list_pending_candidates.py` — CLI (promote, promote-and-ab, list --sort value --high-value-only)
- `/home/agx/agentforge/bin/rust_flywheel_after_task.sh` — robust rate-limited (5min flock) hook for every real task completion
- `/home/agx/agentforge/bin/rust_post_process_hook.py` — Python post_process integration
- `/home/agx/agentforge/bin/enable_rust_flywheel.sh` + `/home/agx/agentforge/bin/rust_flywheel.env`
- `/home/agx/agentforge/ENABLE_RUST_FLYWHEEL` (marker file containing "1") + `ENABLE_RUST_FLYWHEEL.md`

### Continuous Autonomy + 24/7 Self-Improvement Timer (Final Closer)
- `/home/agx/agentforge/bin/run_continuous_flywheel.py` + `.sh` — prioritizer + promote-and-ab + winner detection (flock + fcntl, dry-run default, 240s timeout)
- `/home/agx/agentforge/agentforge-flywheel.service` + `/home/agx/agentforge/agentforge-flywheel.timer` (20min cadence)
- `/home/agx/agentforge/bin/cron_continuous_flywheel.example`
- `/home/agx/agentforge/bin/execute_real_abs_on_promoted.sh` — master script to flip 3 promoted candidates to real A/B mode + execute
- `/home/agx/agentforge/bin/real_ab_farm_commands.txt` — full copy-paste real A/B blocks (rate cleanup, n=3, timeout, monitoring)
...
- `/home/agx/agentforge/pending_candidates/` — 236 directories total (as of 2026-05-31 09:xx)
...
- **Promoted Skill Artifacts (safe timestamped + canonical):**
  ...
  - `/home/agx/agentforge/skills/general-refactor-flywheel.yaml` (canonical production variant)
- **Promotion Indexes:**
  - `/home/agx/agentforge/skills/promotion_history.json` (4+ entries, .bak)
...
- `/home/agx/agentforge/IMPACT_REPORT.md` — full dashboard (236 candidates, 142 rich, 2879 total high_learning_value_records, rich_success_rate avg~0.0199 max 0.0813, 3 A/Bs, projections for real lift +5-15pp, exact verification commands)
- `/home/agx/agentforge/CONTINUOUS_FLYWHEEL.md` — 24/7 autonomy architecture + enable commands
- `/home/agx/agentforge/FARM_ROLLOUT_CHECKLIST.md` — production ops runbook (systemd envs, timer, monitoring, rollback)
- `/home/agx/agentforge/AGENTFORGE_FRONTIER_ROADMAP.md` — updated with 100% ACHIEVED declarations
- `/home/agx/agentforge/PENDING_CANDIDATES.md` — living log of all tracks + agent swarm history (multiple sections on parallel Jules agents)
...
- Other: `/home/agx/agentforge/eval/trajectories/`, prm sidecars, logs/continuous_flywheel.log, watchdog.py updates
