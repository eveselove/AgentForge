...
pub fn promote_candidate(
    ...
    if copy_to_skills && yaml_src.exists() {
        let skills_dir = std::env::var("AGENTFORGE_SKILLS_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from("/home/agx/agentforge/skills"));
    ...
    let skills_dir = std::env::var("AGENTFORGE_SKILLS_DIR").map(PathBuf::from).unwrap_or_else(|_| PathBuf::from("/home/agx/agentforge/skills"));
