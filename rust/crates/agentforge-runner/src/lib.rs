...
mod tests {
    ...
    fn cli_binary_help_and_version_exec() {
        ...
        let bin = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../target/debug/agentforge-runner");
        if !bin.exists() {
        ...
        for fmt in ["pairs", "full", "stats", "prm-steps"] {
            let out = std::process::Command::new(&bin)
                .args([
                    ...
                    traj_dir,
                    "--output",
                    &format!("/tmp/test_flywheel_{}.jsonl", fmt),
                    "--format",
                    fmt,
                ])
                .output()
                .expect("exec flywheel-export");
            assert!(
                out.status.success(),
                "flywheel-export --format {} failed",
                fmt
            );
            let s = String::from_utf8_lossy(&out.stdout);
            assert!(
                s.contains("\"cmd\":\"flywheel-export\""),
                "missing cmd in {}",
                fmt
            );
            assert!(
                s.contains(&format!("\"format\":\"{}\"", fmt)) || s.contains("\"format\""),
                "format not echoed for {}",
                fmt
            );
        }
    ...
    fn cli_flywheel_step_shadow_support_and_emission() {
        ...
        let out = std::process::Command::new(&bin)
            ...
            .output()
            .expect("exec flywheel-step with shadow");
        assert!(out.status.success(), "shadow flywheel-step failed");
        let s = String::from_utf8_lossy(&out.stdout);
        assert!(
            s.contains("\"shadow\":true"),
            "shadow flag must be reflected in JSON output"
        );
        assert!(s.contains("\"cmd\":\"flywheel-step\""));
        // Emission artifacts from orchestrator
        assert!(
            std::path::Path::new(&tmp_out)
                .join("proposal.json")
                .exists()
                || s.contains("proposal")
        );
    ...
    fn cli_candidate_promote_and_continuous_integration_smoke() {
        ...
        let promote_help = std::process::Command::new(&bin)
            ...
            .output()
            .expect("candidate promote usage");
        // It may error on missing id but must not panic/crash binary
        let ph = format!(
            "{}{}",
            String::from_utf8_lossy(&promote_help.stdout),
            String::from_utf8_lossy(&promote_help.stderr)
        );
        assert!(
            ph.contains("candidate promote")
                || ph.contains("missing candidate_id")
                || promote_help.status.success()
        );
    ...
    fn cli_candidate_promote_dry_run_on_real_pending_data() {
        ...
        let out = std::process::Command::new(&bin)
            ...
            .output()
            .expect("exec candidate promote --dry-run on real id");

        // tolerate if candidate not present in this env (promote exits 1 on missing, but surface exercised; don't crash)
        let s = String::from_utf8_lossy(&out.stdout);
        let se = String::from_utf8_lossy(&out.stderr);
        let combined = format!("{}{}", s, se);
        assert!(
            out.status.success()
                || combined.contains("error")
                || combined.contains("promote")
                || combined.contains("candidate"),
            "promote dry-run must not panic (exit or error json ok if candidate absent)"
        );
        // The json emitted by promote path in main (or error case)
        assert!(
            s.contains("\"cmd\":\"candidate promote\"")
                || s.contains("candidate promote")
                || combined.contains("promote_failed")
                || combined.contains("missing candidate_id")
        );
        // Dry run or error path reported
        assert!(
            s.contains("dry_run")
                || s.contains("--dry-run")
                || s.contains("would")
        ...
        let cont = std::process::Command::new(&bin)
            ...
            .output()
            .expect("continuous --no-dry-run parse");
    ...
    fn cli_promote_and_continuous_full_fields_and_disable_dry_paths() {
        ...
        let p = std::process::Command::new(&bin)
            ...
            .output()
            .expect("promote full fields");
        let ps = String::from_utf8_lossy(&p.stdout);
        let pe = String::from_utf8_lossy(&p.stderr);
        assert!(
            p.status.success()
                || ps.contains("error")
                || ps.contains("promote")
                || pe.contains("promote"),
            "promote status or graceful error on possibly-absent candidate"
        );
        assert!(
            ps.contains(real_id)
                || ps.contains("promoted")
                || ps.contains("error")
                || ps.contains("missing")
        );
    ...
    fn cli_flywheel_step_real_data_limit_and_disable_graceful_emission() {
        ...
        let out = std::process::Command::new(&bin)
            ...
            .output()
            .expect("real limit path");
        assert!(out.status.success());
        let s = String::from_utf8_lossy(&out.stdout);
        assert!(s.contains("\"cmd\":\"flywheel-step\""));
        // Proposals + analysis still emitted even with small limit (improved emission)
        assert!(
            s.contains("proposals") || s.contains("overall_rationale") || s.contains("analysis")
        );
    }
        ...
        fn prop_cli_continuous_various_top_n_and_shadow_flags_parse_and_emit_health(
            ...
            if bin.exists() {
                ...
                if let Ok(o) = out {
                    ...
                    let expected_top = format!("\"top_n\":{}", top_n);
                    prop_assert!(s.contains(&expected_top) || s.contains("top_n"));
                    if use_shadow {
    ...
    fn runner_cli_promote_continuous_shadow_cross_full_cycle_smoke() {
        ...
        let step = std::process::Command::new(&bin)
            ...
            .output()
            .expect("cross step shadow");
        assert!(step.status.success());
        let ss = String::from_utf8_lossy(&step.stdout);
        assert!(
            ss.contains("\"shadow\":true")
                && (ss.contains("CRITIQUE") || ss.contains("proposal") || ss.contains("emission"))
        );
        ...
        let prom = std::process::Command::new(&bin)
            ...
            .output()
            .expect("cross promote");
        let ps = String::from_utf8_lossy(&prom.stdout);
        let pe = String::from_utf8_lossy(&prom.stderr);
        assert!(
            prom.status.success()
                || ps.contains("error")
                || ps.contains("promote")
                || pe.contains("promote"),
            "cross promote graceful on absent candidate ok"
        );
        assert!(
            ps.contains(pid)
                || ps.contains("promoted")
    ...
    fn runner_continuous_health_json_contains_all_new_component_fields_for_shadow_parity() {
        ...
        // ensure fresh write
        let _ = std::fs::remove_file("/tmp/agentforge_rust_flywheel/flywheel_health.json");
        ...
        {
            // continuous with top0 still emits suggested (empty), source, etc for parity
            if h.contains("continuous") || h.contains("source") {
                assert!(
                    h.contains("shadow") && h.contains("dry_run") && h.contains("suggested"),
                    "health must include core fields when freshly written by continuous"
                );
