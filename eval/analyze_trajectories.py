...
This file is CORE EVAL (prm/trajectory analysis) — EXEMPT from Phase 4 Python flywheel orchestration removal.
Only flywheel glue (e.g. in post_process.py, runner.py orchestration entrypoints) is deprecated.
Trajectory analysis for pure eval paths remains fully supported + independent of flywheel proposal/candidate/continuous.
Guard logic + banners apply only to orchestration (see learning/utils.py + PHASE4_* docs for full targets list).
Non-orchestration eval/ is out of Phase 4 flywheel scope (see PLAN exemptions).
