...
def execute_build(task):
    ...
    if not git_branch:
        api_request(
            ...
            {
                "status": "failed",
                "result": "No git_branch specified for build",
    ...
    try:
        ...
        subprocess.run(
            ["git", "worktree", "add", worktree_dir, git_branch],
            cwd=PROJECT_DIR,
            check=False,
            capture_output=True,
        )
        ...
        api_request(
            "PATCH",
            f"/tasks/{task_id}",
            {"status": "failed", "result": "Git worktree error"},
        )
        return
    ...
    if exit_code == 0:
        ...
        api_request(
            ...
            {
                "status": "review",
                "result": "Build and tests passed successfully",
                "assigned_agent": AGENT_ID,
        ...
        api_request(
            ...
            {
                "status": "failed",
                "result": f"Build failed (exit {exit_code})",
                "assigned_agent": AGENT_ID,
                "duration_seconds": round(duration, 1),
            },
        )
