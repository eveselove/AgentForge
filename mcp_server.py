    ...
    return [
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "status": {"type": "string", "description": "Фильтр по статусу (pending, in_progress, dispatched, review, done, failed)"}
                }
            }
        ),
        Tool(
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "task_id": {"type": "string"},
                    "agent": {"type": "string", "description": "ID агента (например, grok, jules, antigravity-subagent)"}
                },
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "task_id": {"type": "string"},
                    "status": {"type": "string", "enum": ["pending", "in_progress", "dispatched", "review", "done", "failed"]},
                    "result": {"type": "string", "description": "Результат выполнения (markdown/text)"},
    ...
    try:
        if name == "agentforge_list_tasks":
            ...
            for t in tasks:
                ...
                icon = "⏳" if stat == "pending" else "🚀" if stat in ["in_progress", "dispatched"] else "✅" if stat == "done" else "👁️" if stat == "review" else "❌"
                desc = str(t.get('description', '')).split('\n')[0][:80]
                agent = t.get('assigned_agent', 'None')
                
                output += f"{icon} [{t_id}] {desc}... | статус: {stat} | агент: {agent}\n"
            ...
            if "status" in res and res["status"] == "dispatched":
                return [TextContent(type="text", text=f"📤 Задача {task_id[:8]} отправлена агенту: {agent}\nСтатус: dispatched")]
            return [TextContent(type="text", text=json.dumps(res, ensure_ascii=False, indent=2))]
            ...
            if "id" not in t:
                return [TextContent(type="text", text=json.dumps(t, ensure_ascii=False))]
        ...
        return [TextContent(type="text", text=f"Ошибка выполнения инструмента: {str(e)}")]
