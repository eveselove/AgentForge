...
**Phase 1 Deprecation Expander (turbo agent):** Added deprecation warnings + TODOs (all referencing RUST_FULL_MIGRATION_PLAN.md) + updated module docstrings to next batch of orchestration files from inventory: `bin/run_continuous_flywheel.py`, `eval/runner.py`, `bin/rust_flywheel_after_task.sh`, `bin/run_continuous_flywheel.sh`, `DELETED (Tier2) - direct runner`, `list_pending_candidates.py`, and `learning/evaluator.py`. Expanded `learning/utils.py:is_pure_rust_flywheel()` usage (imports + guards + conditionals) into the continuous runner, post-process hook, and list_pending CLI. All changes conservative/non-breaking (warnings only on Python paths when not pure; existing ENABLE semantics untouched; no behavior/artifact changes). See updated headers + the central helper for details. Next: more hooks + Rust parity scaffolding.
...
3. Also wired through:
   ...
   - `DELETED (Tier2) - direct runner` (when both env flags set)
