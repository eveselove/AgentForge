...
- Bridge hardening agent: **Direct Rust flywheel-step bridge in eval/post_process.py + DEPRECATED (Tier 2 surgical, see docs/JULES_PY_REMOVAL_HANDOFF_f29c675b.md and PHASE4 checklist)**. When `is_pure_rust_flywheel()` / `AGENTFORGE_FLYWHEEL_ENGINE=rust` / marker: **always prefers** `./rust/target/release/agentforge-runner flywheel-step --real-data ... --ingest`. Logging: "[post_process] Using PURE RUST flywheel-step via agentforge-runner". Python path now explicit fallback. Non-breaking.
