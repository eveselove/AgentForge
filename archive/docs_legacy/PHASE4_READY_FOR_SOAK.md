...
- Call-site classification: ZERO ACTIVE_PROD stray outside tiers (the few "stray" detections were expected bridge imports inside eval/post_process.py:378 and DELETED (Tier2) - direct runner:62/222 — these files are Tier 2/3 surgical targets).
...
- High-care files: eval/post_process.py:148-397 (PURE RUST BRIDGE + legacy fallback; delicate rate-limit + do_flywheel logic); DELETED (Tier2) - direct runner:38/111 (guard + trigger).
