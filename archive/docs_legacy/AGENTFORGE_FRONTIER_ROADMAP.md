...
- All workers patched: `jules_worker.sh`, `grok_worker.sh`, `agents/grok_runner.sh`, `dispatcher.sh` + `bin/rust_flywheel_after_task.sh` + `DELETED (Tier2) - direct runner` respect `ENABLE_RUST_FLYWHEEL` marker + env; call post_process + canonical step.
