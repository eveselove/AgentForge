...
if __name__ == "__main__":
    print(_MESSAGE)
    # task-5af0e350: direct to runner for pure default (if available)
    import os
    import sys
    ...
    if os.path.isfile(runner) and os.access(runner, os.X_OK):
        # forward args for flywheel-step etc, but since this was step, default to step
        args = [
            runner,
            "--json",
            "flywheel-step",
        ...
        print(f"  Directing to pure runner: {' '.join(args)}")
        try:
